...
const GrainContracts_1 = require("../../app/entities/GrainContracts");
async function updateContractEmissionDatetime(dataSource) {
    const grainContractRepository = dataSource.getRepository(GrainContracts_1.GrainContract);
    try {
        ...
        for (const contract of contracts) {
            try {
                ...
                if (!contract.contract_emission_date) {
                    console.warn(`Contrato ${contract.id} não possui contract_emission_date`);
                    skipped++;
                    continue;
                }
                ...
                console.error(`Erro ao atualizar contrato ${contract.id}:`, error.message);
                skipped++;
            }
        }
        console.log(`
