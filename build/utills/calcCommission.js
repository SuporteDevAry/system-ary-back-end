...
function calcCommission(contract) {
    ...
    const totalValue = typeof contract.total_contract_value === "string"
    ...
    const commission = contract.commission_seller || contract.commission_buyer || 0;
    const typeCommission = contract.type_commission_seller || contract.type_commission_buyer || "";
    const commissionNumber = Number(typeof commission === "string" ? commission.replace(",", ".") : commission);
    // Calcula a comissão
    const commissionValue = typeCommission === "Percentual"
