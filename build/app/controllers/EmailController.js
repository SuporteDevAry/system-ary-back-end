...
const EmailLogRepository_1 = require("../repositories/EmailLogRepository");
const coverPage_1 = require("../../pdfGenerator/helpers/coverPage");
const GrainContractRepository_1 = require("../repositories/GrainContractRepository");
const signatureEmailImageSoy = path_1.default.resolve(__dirname, "../../pdfGenerator/helpers/assinatura_execucao_mi.png");
const signatureEmailImageOil = path_1.default.resolve(__dirname, "../../pdfGenerator/helpers/assinatura_oleo.png");
    ...
    async SendEmails(req, res) {
        ...
        try {
            ...
            if (contractData.id &&
                ...
                const dbContract = await GrainContractRepository_1.grainContractRepository.findOne({
            ...
            const nameSeller = ((_a = contractData.seller) === null || _a === void 0 ? void 0 : _a.nickname)
                ? contractData.seller.nickname
                : contractData.seller.name;
            const nameBuyer = ((_b = contractData.buyer) === null || _b === void 0 ? void 0 : _b.nickname)
                ? contractData.buyer.nickname
                : contractData.buyer.name;
            // Gerar o PDF para o vendedor
            const pdfSeller = await (0, pdfGenerator_1.default)({
                data: contractData,
                typeContract: "Vendedor",
                template: templateName,
            });
            // Gerar o PDF para o comprador
            const pdfBuyer = await (0, pdfGenerator_1.default)({
                data: contractData,
          ...
          <div style="font-family: 'Courier New', Courier, monospace, Arial, sans-serif; font-weight: 400; color: rgb(0, 0, 0); font-size: 14px; line-height: 21px;">
            ...
            await EmailLogRepository_1.EmailLogRepository.save({
                email_sender: sender,
                number_contract,
                sent_at: new Date(),
            });
