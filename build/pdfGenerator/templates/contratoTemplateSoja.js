...
const Extenso_1 = require("../helpers/Extenso");
const logoContrato = path_1.default.resolve(__dirname, "../helpers/Logo_Ary_Completo.jpg");
const logoBase64 = `data:image/jpeg;base64,${fs_1.default
    .readFileSync(logoContrato)
    .toString("base64")}`;
const ContratoTemplateSoja = ({ data, typeContract, modeSave, }) => {
    var _a, _b, _c;
    ...
    const { seller, buyer, number_contract, product, number_broker, quantity, commission_seller, commission_buyer, quality, price, type_currency, complement_destination, destination, icms, payment, pickup, pickup_location, inspection, observation, type_quantity, name_product, type_commission_seller, type_commission_buyer, type_commission_seller_currency, type_commission_buyer_currency, type_pickup, } = data;
    const formatCommissionAmount = (value, currency) => {
        ...
        const isDollarCurrency = currency === "Dólar" || currency === "USD" || currency === "US$";
        const currencySymbol = isDollarCurrency ? "$" : "R$";
        return `${currencySymbol} ${parsedValue.toFixed(2).replace(".", ",")}`;
    };
    const quantityValue = typeof quantity === "number" ? quantity : (0, helpers_1.parseQuantityToNumber)(quantity);
    const formattedQtd = (0, helpers_1.numberToQuantityString)(quantity);
    ...
    if (type_quantity === "toneladas métricas") {
        ...
        const kilosText = kilosFromDecimals > 0
        ...
        const ext = (0, Extenso_1.Extenso)(inteiro, "M");
        formattedExtenso = `(${ext})`;
    }
    let formattedSellerCNPJ = (seller === null || seller === void 0 ? void 0 : seller.cnpj_cpf)
        ? (0, helpers_1.insertMaskInCnpj)(seller.cnpj_cpf)
        : "";
    let formattedBuyerCNPJ = (buyer === null || buyer === void 0 ? void 0 : buyer.cnpj_cpf)
    ...
    if (commission_seller) {
        if (type_commission_seller === "Percentual") {
            ...
            const formattedAmount = formatCommissionAmount(commission_seller, type_commission_seller_currency || "Real");
            formattedCSeller =
    ...
    if (commission_buyer) {
        if (type_commission_buyer === "Percentual") {
            ...
            const formattedAmount = formatCommissionAmount(commission_buyer, type_commission_buyer_currency || "Real");
            formattedCBuyer =
    ...
    let formattedSafra = validProductsForMetricTon
        ? ` `
        : ` - Safra: ${data.crop}`;
    let formattedMetrica = data.type_quantity === "toneladas métricas" ? `.` : ` quilos.`;
    let Dot = data.destination === "Nenhum" ||
        (data.destination === "" && ((_a = data.complement_destination) === null || _a === void 0 ? void 0 : _a.length) === 0)
        ? "."
        : ", ";
    let formattedPreco = data.type_quantity === "toneladas métricas"
        ? ` por tonelada métrica${Dot}`
        : ` por saca de 60(sessenta) quilos${Dot}`;
    let formattedComplementSeller = ((_b = data.seller) === null || _b === void 0 ? void 0 : _b.complement)
        ? `${" - "} ${data.seller.complement} `
        : "";
    let formattedComplementBuyer = ((_c = data.buyer) === null || _c === void 0 ? void 0 : _c.complement)
    ...
    return (react_1.default.createElement("div", { id: "contrato" },
        ...
        react_1.default.createElement("div", { style: { textAlign: "justify" } },
            react_1.default.createElement("strong", null, data.type_currency === "Dólar"
                ? `${(0, helpers_1.formatCurrency)(price, data.type_currency, modeSave).replace("$", "US$ ")}`
